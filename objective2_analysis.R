# ============================================================================
# OBJECTIVE 2: Effect of Sustainability Disclosure Quality (SDQ) on Firm Value
...
# extended checks (two-way FE, lagged SDQ), and produces both figures used
...
#   ./outputs/objective2/   <- created automatically if missing
#
# Run with:   Rscript objective2_analysis.R
# ============================================================================
...
output_dir <- file.path("outputs", "objective2")
...
vars <- c("Ticker", "Year", "SDQ_Score (0-100)", "Tobin_Q_Wins",
          "Firm_Size (LN Total Assets)", "Leverage_Wins", "Firm_Age",
          "IFRS_S1S2_Adopter_Dummy")
df <- df_raw[, vars]
names(df) <- c("Ticker", "Year", "SDQ_Score", "Tobin_Q_Wins", "Firm_Size",
               "Leverage_Wins", "Firm_Age", "EAS_Dummy")
df <- na.omit(df)

write.csv(df, file.path(output_dir, "objective2_panel_data.csv"), row.names = FALSE)
...
fe_base <- plm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age,
               data = pdata, model = "within")
re_base <- plm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age,
...
write.csv(model_overview, file.path(output_dir, "objective2_model_overview.csv"), row.names = FALSE)
...
write.csv(fe_results_table, file.path(output_dir, "objective2_fe_results.csv"), row.names = FALSE)
...
write.csv(re_results_table, file.path(output_dir, "objective2_re_results.csv"), row.names = FALSE)

## ---- 4. Diagnostics ----------------------------------------------------------
# 4a. Correlation matrix among the regressors
cor_vars   <- df[, c("SDQ_Score", "Firm_Size", "Leverage_Wins", "Firm_Age")]
cor_matrix <- cor(cor_vars, use = "pairwise.complete.obs", method = "pearson")
write.csv(round(cor_matrix, 3), file.path(output_dir, "objective2_correlation_matrix.csv"))
...
pooled_ols <- lm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age, data = df)
vif_vals   <- car::vif(pooled_ols)
write.csv(data.frame(Variable = names(vif_vals), VIF = round(vif_vals, 3)),
          file.path(output_dir, "objective2_vif.csv"), row.names = FALSE)

# 4c. Breusch-Pagan test for heteroskedasticity (regressors as in the base model)
bp_test <- bptest(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age, data = df)
...
write.csv(diagnostics_summary, file.path(output_dir, "objective2_diagnostics.csv"), row.names = FALSE)

## ---- 5. Robustness checks 1-4 ------------------------------------------------
# Check 1: Non-linearity (quadratic SDQ term)
fe_nonlin <- plm(Tobin_Q_Wins ~ SDQ_Score + I(SDQ_Score^2) + Firm_Size + Leverage_Wins + Firm_Age,
                  data = pdata, model = "within")
nonlin_clustered <- coeftest(fe_nonlin, vcov = vcovHC(fe_nonlin, method = "arellano",
                                                        type = "HC1", cluster = "group"))
cat("\n================ CHECK 1: NON-LINEARITY (SDQ + SDQ^2) ================\n"); print(nonlin_clustered)

# Check 2: Moderator, SDQ x Early Adoption Status
fe_interact <- plm(Tobin_Q_Wins ~ SDQ_Score * EAS_Dummy + Firm_Size + Leverage_Wins + Firm_Age,
                    data = pdata, model = "within")
interact_clustered <- coeftest(fe_interact, vcov = vcovHC(fe_interact, method = "arellano",
                                                            type = "HC1", cluster = "group"))
cat("\n================ CHECK 2: MODERATOR (SDQ x EAS_Dummy) ================\n"); print(interact_clustered)
...
fe_excl    <- plm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age,
...
robustness_results <- rbind(
  extract_row(base_clustered,    "SDQ_Score",              "1. Base model"),
  extract_row(nonlin_clustered,  "SDQ_Score",               "2. Non-linearity: linear term"),
  extract_row(nonlin_clustered,  "I(SDQ_Score^2)",          "2. Non-linearity: quadratic term"),
  extract_row(interact_clustered,"SDQ_Score",               "3. Moderator: SDQ main effect"),
  extract_row(interact_clustered,"EAS_Dummy",               "3. Moderator: EAS main effect"),
  extract_row(interact_clustered,"SDQ_Score:EAS_Dummy",     "3. Moderator: SDQ x EAS interaction"),
  extract_row(excl_clustered,    "SDQ_Score",               "4. Excluding early-adopter firms")
)
write.csv(robustness_results, file.path(output_dir, "objective2_robustness.csv"), row.names = FALSE)
...
write.csv(n_info, file.path(output_dir, "objective2_robustness_n.csv"), row.names = FALSE)

## ---- 6. Extended checks: two-way FE, lagged SDQ ------------------------------
...
fe_oneway <- plm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age,
                  data = pdata, model = "within", effect = "individual")
fe_twoway <- plm(Tobin_Q_Wins ~ SDQ_Score + Firm_Size + Leverage_Wins + Firm_Age,
...
# Extended Check 2: lagged SDQ (t-1), to address reverse-causality concerns
pdata$SDQ_lag1 <- plm::lag(pdata$SDQ_Score, 1)
fe_lagged <- plm(Tobin_Q_Wins ~ SDQ_lag1 + Firm_Size + Leverage_Wins + Firm_Age,
                  data = pdata, model = "within")
lagged_clustered <- coeftest(fe_lagged, vcov = vcovHC(fe_lagged, method = "arellano",
                                                        type = "HC1", cluster = "group"))
cat("\n================ LAGGED SDQ MODEL (firm-clustered SE) ================\n"); print(lagged_clustered)
cat("\nN in lagged model:", nobs(fe_lagged), "(vs", nobs(fe_oneway), "in base model, lost to lag construction)\n")

extended_results <- rbind(
  extract_row(twoway_clustered, "SDQ_Score",     "5. Two-way FE (firm+year)"),
  ...
  extract_row(lagged_clustered, "SDQ_lag1",      "6. Lagged SDQ (t-1)"),
  extract_row(lagged_clustered, "Firm_Size",     "6. Lagged SDQ (t-1)"),
  extract_row(lagged_clustered, "Leverage_Wins", "6. Lagged SDQ (t-1)"),
  extract_row(lagged_clustered, "Firm_Age",      "6. Lagged SDQ (t-1)")
)
write.csv(extended_results, file.path(output_dir, "objective2_extended_checks.csv"), row.names = FALSE)
...
write.csv(diag_extra, file.path(output_dir, "objective2_extended_diagnostics.csv"), row.names = FALSE)

n_extra <- data.frame(
  Model     = c("One-way FE (base)", "Two-way FE", "Lagged SDQ FE"),
...
write.csv(n_extra, file.path(output_dir, "objective2_extended_n.csv"), row.names = FALSE)

## ---- 7. Secondary descriptive figure: raw scatter + partial regression plot --
pA <- ggplot(df, aes(x = SDQ_Score, y = Tobin_Q_Wins)) +
  ...
  labs(title = "A. Raw Scatter (pooled)",
       ...
       x = "SDQ Score", y = "Tobin's Q (winsorized)") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))

# Partial regression (added-variable) plot via Frisch-Waugh-Lovell: residualize
# both SDQ and Tobin's Q on firm FE + other covariates; the slope of
# resid(Y) ~ resid(X) reproduces the SDQ coefficient from the full FE model.
fe_y <- plm(Tobin_Q_Wins ~ Firm_Size + Leverage_Wins + Firm_Age, data = pdata, model = "within")
fe_x <- plm(SDQ_Score    ~ Firm_Size + Leverage_Wins + Firm_Age, data = pdata, model = "within")
resid_df  <- data.frame(y_resid = residuals(fe_y), x_resid = residuals(fe_x))
fwl_check <- lm(y_resid ~ x_resid, data = resid_df)
cat("\nFWL slope check (should match the FE model's SDQ coefficient,",
    round(base_clustered["SDQ_Score", 1], 5), "):\n")
...
pB <- ggplot(resid_df, aes(x = x_resid, y = y_resid)) +
  ...
  labs(title = "B. Partial Regression Plot",
       ...
       x = "SDQ Score (residualized)", y = "Tobin's Q (residualized)") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))

combined <- pA + pB +
  plot_annotation(title = "Objective 2: SDQ and Firm Value (Tobin's Q)",
                   theme = theme(plot.title = element_text(face = "bold", size = 15)))
ggsave(file.path(output_dir, "objective2_scatter_combined.png"), combined,
       width = 13, height = 5.5, dpi = 150)
cat("\nSaved secondary descriptive figure: objective2_scatter_combined.png\n")

## ---- 8. Primary figure: SDQ + controls across the three core specifications --
...
primary_data <- list(
  SDQ_Score     = build_spec_df("SDQ_Score", "SDQ_lag1"),
...
# RIGHT_TEXT_FRAC bumped from 0.62 -> 0.70 in this retrofit: the coefficient
# text labels are rendered bigger now (size 3.6 vs the original 3.3), and at
# 0.62 the widest label ("0.319  (p<0.001)" in the Firm_Age panel) clipped
# against the right edge of the image. 0.70 reserves enough room at the
# larger font size; verified by re-rendering.
LEFT_TEXT_FRAC  <- 0.28
RIGHT_TEXT_FRAC <- 0.70
...
make_panel <- function(var_name, is_primary = FALSE, show_xlab = FALSE, show_legend = FALSE) {
  ...
  ggplot(d, aes(y = spec)) +
    ...
    # a panel missing e.g. the marginal/orange color (Firm_Age has none) would
    # silently drop that key even though it was named in breaks/labels.
    ...
    # SDQ) closer together while keeping each variable's heading close to its
    # own bars and visually distinct from the next variable's block -- ported
    # directly from Objective 3's figure, which received this same treatment
    # first (see that script/its rework notes for the full reasoning). add is
    # c(low, high): "low" sits next to level 1 of the (reversed) factor --
    # "Lagged (t-1)", at the BOTTOM of each panel -- and "high" sits next to
    # the top level, "Base FE", right under this panel's own heading. A small
    # "high" value keeps the heading tight to its first bar; a larger "low"
    ...
    labs(title = if (is_primary) expression(bold("SDQ_Score   PRIMARY VARIABLE OF INTEREST"))
                 else var_name,
         x = if (show_xlab) "Coefficient on Tobin's Q" else NULL, y = NULL) +
    theme_minimal(base_size = 12) +
    theme(
      # margin: small top gap above the heading text, near-zero bottom gap,
      # so the heading sits right on top of its own bars.
      ...
      # SDQ) -- bumped up from theme_minimal's default (~9.6pt), matching
      # Objective 3's figure.
      axis.text.y = element_text(size = 12),
      axis.text.x = element_text(size = 9.5),
      axis.title.x = element_text(size = 13),
      # axis.line.x / axis.ticks.x: theme_minimal() draws neither by default;
      # added back to match the original workbook's figure style (a thin
      # horizontal rule under each panel's x-axis tick labels).
      ...
      # heading-close-to-own-bars / clear-gap-before-next-variable effect at
      # the whole-panel level (the gap between one panel's plot area and the
      # next, once patchwork stacks them).
...
panels <- list(
  make_panel("SDQ_Score", is_primary = TRUE),
...
primary_fig <- wrap_plots(panels, ncol = 1) +
  plot_annotation(
    title = "Objective 2: SDQ and Control Variables Across Core Specifications",
    subtitle = paste0(
      "Fixed Effects, firm-clustered SE (Petersen, 2009), 95% CI — zero reference ",
      "line aligned across panels\nx-axis scale shared with the companion CDQ figure ",
      "(Objective 3)"
    ),
    caption = paste0(
  ...
  # asymmetry with one uniform value on every panel. Matches Objective 3's
  # figure code.

ggsave(file.path(output_dir, "objective2_primary_figure.png"), primary_fig,
       width = 9, height = 13.6, dpi = 200)
cat("\nSaved primary figure: objective2_primary_figure.png\n")

## ---- 9. Reproducibility record -----------------------------------------------
writeLines(capture.output(sessionInfo()), file.path(output_dir, "session_info.txt"))

## ---- 10. Consolidated workbook: mirrors NGX_Stage2_Objective2_SDQ_FirmValue.xlsx --
...
writeData(wb, "1. Model Overview", "OBJECTIVE 2: SDQ AND FIRM VALUE", startRow = 1, colNames = FALSE)
...
writeData(wb, "1. Model Overview",
          "Tobin_Q_it = b0 + b1(SDQ_Score_it) + b2(Firm_Size_it) + b3(Leverage_it) + b4(Firm_Age_it) + e_it",
...
writeData(wb, "5. Plots", "OBJECTIVE 2: PRIMARY FIGURE AND SUPPORTING DESCRIPTIVE FIGURE",
...
writeData(wb, "5. Plots",
          paste0("SDQ_Score and all control variables (Firm_Size, Leverage_Wins, Firm_Age) across the three ",
                 "core specifications: Base FE, Two-way FE, and Lagged SDQ (t-1). Each variable keeps its own ",
                 "x-axis scale, but the zero reference line is forced to the same horizontal position in every ",
                 "panel so it reads as one continuous line down the figure. SDQ_Score is the primary variable ",
                 ...
                 "end of each bar, the significance star at the other. x-axis scale is intended to match the ",
                 "companion CDQ figure (Objective 3) if that figure is regenerated with the same alignment script."),
          startRow = 4, colNames = FALSE)
insertImage(wb, "5. Plots", file.path(output_dir, "objective2_primary_figure.png"),
...
writeData(wb, "5. Plots",
          paste0("Panel A: raw pooled scatter, illustrative only. Panel B: partial regression plot; its slope ",
                 "reproduces the FE model's SDQ coefficient exactly, verified via the Frisch-Waugh-Lovell theorem."),
          startRow = 73, colNames = FALSE)
insertImage(wb, "5. Plots", file.path(output_dir, "objective2_scatter_combined.png"),
...
writeData(wb, "7. Extended Checks", "EXTENDED CHECKS: TWO-WAY FE AND LAGGED SDQ", startRow = 1, colNames = FALSE)
...
for (i in seq_len(nrow(extended_results))) {
  ...
  if (rr$Variable %in% c("SDQ_Score", "SDQ_lag1")) {
...
writeData(wb, "7. Extended Checks",
          sprintf("Lagged SDQ FE: N=%d, within R2=%.3f", nobs(fe_lagged), summary(fe_lagged)$r.squared[1]),
...
# is still named "objective2_analysis.R" and is being run from its own
...
own_source <- tryCatch(readLines("objective2_analysis.R", warn = FALSE),
                        error = function(e) NULL)
if (is.null(own_source)) {
  own_source <- c("(Could not read this script's own source under the name",
                   "'objective2_analysis.R' in the current working directory --",
...
## -- Sheet 9: Panel Data --------------------------------------------------------
addWorksheet(wb, "Panel Data (Obj 2)")
...
writeData(wb, "Panel Data (Obj 2)", panel_export, startRow = 1, startCol = 1,
          colNames = TRUE, rowNames = FALSE)
addStyle(wb, "Panel Data (Obj 2)", style_hdr_navy, rows = 1, cols = 1:ncol(panel_export), gridExpand = TRUE)
setColWidths(wb, "Panel Data (Obj 2)", cols = 1:9, widths = c(10, 6, 18, 10, 10, 12, 10, 13, 9))

workbook_path <- file.path(output_dir, "objective2_consolidated_workbook.xlsx")
...
cat("\nAll Objective 2 outputs written to:", normalizePath(output_dir), "\n")
